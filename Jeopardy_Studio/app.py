...
def static_file(start, path, base):
    try:
    ...
    return response(start, body=raw, content_type=mime, headers=[("Cache-Control", "no-cache")])
...
def application(env, start):
    ...
    try:
        if path == "/" and method == "GET":
            return static_file(start, "index.html", ROOT / "static")
        if path.startswith("/static/") and method == "GET":
            return static_file(start, path[8:], ROOT / "static")
        if path.startswith("/uploads/") and method == "GET":
            return static_file(start, path[9:], UPLOAD_DIR)
